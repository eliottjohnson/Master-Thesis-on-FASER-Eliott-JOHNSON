...
\nomenclature[z-BCR]{BCR}{Bunch Counter Reset}
...
\begin{figure}[htbp!] 
\centering    
\includegraphics[width=1.0\textwidth]{TLBSchema.jpg}
...
\begin{subfigure}{.45\textwidth}
    ...
    \end{tikzpicture}
    \caption{LUT\#3=4\\00000011$\rightarrow$0100}
\end{subfigure}
...
The first step in using the TLB at FASER is to make sure it respects all specifications. To do this, we install the TLB board in a scintillator laboratory at CERN. The setup to test the board consists of a pulse generator, an oscilloscope, the digitizer\footnote{The digitizer's brand and model are CAEN V1730.} and the TLB, see Fig. \ref{fig:TLBtestSetup}. 
...
\begin{figure}[htbp!]
    ...
    \end{tikzpicture}
    \caption[Input Delay schema]{Input Delay Schema. The BCR0 outputs a signal at 11245 Hz which is sent as an input in channel 0 and channel 2. We then see the L1A output. (Not shown on the figure is the LEMO cable connecting the digitizer to the TLB to communicate).}
